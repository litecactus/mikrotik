...
/interface pppoe-client add name=pppoe-out1 disabled=no interface=ether5 user=bthomehub@btbroadband.com password=password add-default-route=yes use-peer-dns=yes
...
/ip firewall filter add chain=input action=drop in-interface=ether5 comment="Drop all other traffic on WAN interface"
...
/ip firewall filter add chain=forward action=drop connection-state=new connection-nat-state=!dstnat in-interface=ether5 comment="Drop traffic from WAN to LAN not NATed"
